    ...
    for (g = 0; g < pull->ngroup; g++)
    {
        ...
        if (pgrp->params.bSliced)
        {
            ...
            for (i = 0; i < pgrp->nat_loc; i++)
            {
                ...
                /* Set nonzero weight for the atoms in the slice, don't change the weights of the other atoms */
                if (x_coord >= pgrp->params.slice_x_min && x_coord <= pgrp->params.slice_x_max)
                {
                    ...
                    /* Set weight differently for smooth or non-smooth slice */
                    if (pgrp->params.bSliceSmooth)
                    {
                        /*       x - slice_center
                         * x_ =  ----------------
                         *        slice_len_half
            ...
            int nat_slice = nat_slice_loc;
            if (cr != NULL && PAR(cr))
            {
#if GMX_MPI
                MPI_Allreduce(&nat_slice_loc, &nat_slice, 1, MPI_INT, MPI_SUM, pull->comm.mpi_comm_com);
#endif
            }
            if (nat_slice == 0)
            {
    ...
    for (i = 0; i < pg->params.nat; i++)
    {
        ...
        if (ii >= start && ii < end)
        {
            ...
            if (pg->nat_loc >= pg->nalloc_loc)
            {
                ...
                if (pg->epgrppbc == epgrppbcCOS || pg->params.weight != NULL || pg->params.bSliced)
                {
